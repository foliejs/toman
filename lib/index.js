...
function generateWeb (spec) {
  ...
  let mockDirectory = path.join(directory, 'mock')
  let mockGetDirectory = path.join(mockDirectory, 'get')
  ...
  let homeDirectory = path.join(appDirectory, 'home')
  let homeStyleDirectory = path.join(homeDirectory, 'style')
  ...
  createProjectDirectory(homeDirectory)
  createProjectDirectory(homeStyleDirectory)
  createProjectDirectory(mockDirectory)
  createProjectDirectory(mockGetDirectory)
  console.log(logSymbols.info, 'This generator can also be run with ' + chalk.yellow(spec.name))
  ...
  writeFileSync('skyfile.mustache', spec, 'skyfile.js')
  writeFileSync('front/tsconfig.mock.mustache', spec, 'tsconfig.mock.json')
  spec['directory'] = testDirectory
  writeFileSync('test.mustache', spec, 'index.test.js')
  spec['directory'] = pm2Directory
  writeFileSync('21.mustache', spec, '21.json')
  ...
  spec['directory'] = utilsDirectory
  writeFileSync('front/utils.api.mustache', spec, 'api.ts')
  writeFileSync('front/utils.http.mustache', spec, 'http.ts')
  writeFileSync('front/utils.index.mustache', spec, 'index.ts')
  writeFileSync('front/utils.wheel-event.mustache', spec, 'wheel-event.ts')
  spec['directory'] = stylesDirectory
  writeFileSync('front/style.common.mustache', spec, 'common.styl')
  writeFileSync('front/style.mixin.mustache', spec, 'mixin.styl')
  spec['directory'] = homeDirectory
  writeFileSync('front/home.component.tsx.mustache', spec, 'home.component.tsx')
  writeFileSync('front/home.container.mustache', spec, 'home.container.tsx')
  writeFileSync('front/home.index.mustache', spec, 'index.tsx')
  spec['directory'] = homeStyleDirectory
  writeFileSync('front/home.component.styl.mustache', spec, 'home.component.styl')
  spec['directory'] = mockGetDirectory
  writeFileSync('front/mock.index.mustache', spec, 'index.ts')
